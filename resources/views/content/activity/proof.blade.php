	...
	<div class="row">
		<div class="col-12" style="min-height: 100vh; overflow-x: hidden;">
			<div class="card mb-4">
				...
				<div class="card-body">
					<form action="{{ route('activityEmployee.store') }}" method="POST" enctype="multipart/form-data">
						@csrf
						<input type="hidden" name="activity_id" value="{{ $activity->id }}">
						<div class="row g-2">
							<div class="col-md-12">
								<div style="display: grid; grid-template-columns: 150px 10px 1fr; gap: 5px; text-align: left;">
						...
						<div class="mb-3 mt-1">
							...
							<input type="file" class="form-control @error('value') is-invalid @enderror" id="proofFile{{ $activity->id }}" name="image" accept="image/*">
							@error('image')
							...
							<img id="imgPreview{{ $activity->id }}" src="{{ asset('storage/' . $activity->proofActivity[0]->image) }}" alt="Foto Saat Ini" class="img-fluid mb-3" style="max-height: 200px;">
							@else
							<p class="mt-2">Preview Foto:</p>
							<img id="imgPreview{{ $activity->id }}" src="#" alt="Preview Foto" class="img-fluid mb-3" style="max-height: 200px; display: none;">
							@endif
						</div>
						...
						<div class="row">
							<div class="col-md-6">
								<div class="mb-3">
									...
									<input type="text" name="value" class="form-control @error('value') is-invalid @enderror" placeholder="Keterangan" value="{{ $activity->proofActivity[0]->value ?? old('value') }}">
									@error('value')
									<div class="invalid-feedback">{{ $message }}</div>
									@enderror
								</div>
							...
							<div class="col-md-6">
								<div class="mb-3">
									...
									{{-- <input type="text" class="form-control" name="advice" value="{{ $advice->advice ?? old('name') }}" placeholder="Saran"> --}}
									<input type="text" name="advice" class="form-control @error('advice') is-invalid @enderror" value="{{ $advice->advice ?? old('name') }}" placeholder="Saran">
									@error('advice')
										<div class="invalid-feedback">{{ $message }}</div>
									@enderror
								</div>
					...
					<div class="mt-4">
						...
						<button type="button" onclick="showPatientModal()" class="btn btn-success btn-sm text-white mt-2" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">
							Tambah Pasien
						</button>
						<button type="button" id="refreshProofTableBtn{{ $activity->id }}" class="btn btn-primary btn-sm text-white mt-2 ms-2" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">
							Refresh
						</button>
						@endif
						...
						<table id="proofTable" class="display small" style="width: 100%;">
							...
							<tbody>
								...
								<tr data-patient="{{ json_encode($patient) }}">
									...
									<td>
										<button type="button" class="btn-edit-patient-activity btn btn-primary btn-sm text-white font-weight-bold" data-des="{{ $item->description }}" data-id="{{ $item->id }}"
											style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">
											<i class="fas fa-edit"></i>
										</button>
										<a href="{{route('activityEmployee.deleteActivityPatient', $item->id)}}" class="btn btn-danger btn-sm text-white font-weight-bold" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;"
...
<script>
	...
	$('#proofTable').on('click', '.btn-edit-patient-activity', function() {
		...
		$('#patientModal-notes').val(description);
	...
	$('#patientTable').on('click', '.btn-select-patient', function() {
		...
		$(this).closest('.modal-content').find('.modal-body').animate({
			scrollTop: 0
		}, 'fast');
	});
